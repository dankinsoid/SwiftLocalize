...
/// Bare language subtags that get the O(1) hot-path dictionary. Everything else
/// in `likelySubtags` is emitted as a sorted `[(String, String)]` for binary
/// search. Keeping the hot set small (~40) prevents the generated dictionary
/// literal from blowing up Swift compile times on this file. `und` is included
/// because it's the canonical "unknown" key callers fall back to.
let popularLanguages: Set<String> = [
	"en", "es", "fr", "de", "it", "pt", "nl", "sv", "da", "no",
	"nb", "fi", "pl", "cs", "sk", "hu", "ro", "bg", "hr", "sr",
	"sl", "uk", "ru", "ca", "el", "zh", "ja", "ko", "vi", "th",
	"id", "ms", "hi", "bn", "ar", "fa", "he", "tr", "ur", "und",
]

var likelyCommonPairs: [(String, String)] = []
var likelyRarePairs: [(String, String)] = []
for pair in likelyPairs {
	if popularLanguages.contains(pair.0) {
		likelyCommonPairs.append(pair)
	} else {
		likelyRarePairs.append(pair)
	}
}
...
print("wrote \(outputPath): " +
      "likelySubtagsCommon=\(likelyCommonPairs.count), " +
      "likelySubtagsRare=\(likelyRarePairs.count), " +
