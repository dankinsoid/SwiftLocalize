...
//
// Usage:
//   swift run GeneratePluralRules [<plurals.json|URL> [<ordinals.json|URL>]] [<output.swift>]
//
...
//
// With no source paths, both files are fetched live from the CLDR JSON repo
...
// 0 positional args is the "just give me the latest" path: fetch both
// cardinal and ordinal sources from CLDR. As soon as the caller starts
// passing args we revert to the original positional semantics so that
// `<plurals.json>` alone keeps meaning "no ordinals" (the historical way
// to opt out of ordinal data).
let pluralsSource: String
let ordinalsSource: String?
let outputPath: String

switch positional.count {
case 0:
	pluralsSource = "\(cldrBase)/cldr-core/supplemental/plurals.json"
	ordinalsSource = "\(cldrBase)/cldr-core/supplemental/ordinals.json"
	outputPath = "Sources/SwiftLocalize/PluralRule+Generated.swift"
case 1:
	pluralsSource = positional[0]
	ordinalsSource = nil
	outputPath = "Sources/SwiftLocalize/PluralRule+Generated.swift"
case 2:
	pluralsSource = positional[0]
	ordinalsSource = positional[1]
	outputPath = "Sources/SwiftLocalize/PluralRule+Generated.swift"
case 3:
	pluralsSource = positional[0]
	ordinalsSource = positional[1]
...
default:
	FileHandle.standardError.write(Data(
		"usage: swift run GeneratePluralRules [<plurals.json|URL> [<ordinals.json|URL>]] [<output.swift>]\n".utf8
	))
	exit(2)
}
...
func loadCLDR(_ source: String, kind: String) throws -> [String: [String: String]] {
	...
	guard let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
		...
		throw NSError(domain: "GeneratePluralRules", code: 1, userInfo: [
			NSLocalizedDescriptionKey: "Not a CLDR plurals-type-\(kind) file: \(source)",
		])
	}
	return table
}

let cardinalTable = try loadCLDR(pluralsSource, kind: "cardinal")
let ordinalTable: [String: [String: String]] = try ordinalsSource.map { try loadCLDR($0, kind: "ordinal") } ?? [:]
...
header += "//   swift run GeneratePluralRules [<plurals.json|URL>"
if ordinalsSource != nil { header += " [<ordinals.json|URL>]" }
header += "] [<output.swift>]\n"
header += "//\n"
