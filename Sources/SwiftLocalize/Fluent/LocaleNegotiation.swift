...
public extension Fluent.Tag {
	...
	var canonical: Self {
		...
		if let replacement = Fluent.LocaleData.languageAliases[c.language] {
	...
	var maximized: Self {
		...
		for key in keys {
			guard !key.isEmpty, let hit = Fluent.LocaleData.likelySubtags[key] else { continue }
			...
			return Self(rawValue: merged.bcp47)
		}
		return self
	}
	...
	///   4. Otherwise `nil`.
	var parent: Self? {
		if let p = Fluent.LocaleData.parentLocales[rawValue] {
...
public extension Fluent {
	...
	enum LocaleNegotiation {
		...
		public static func matching(
			requested: [Tag],
			available: [Tag],
			default fallback: Tag? = nil
		) -> [Tag] {
			// Cache canonical + maximized form per available, preserving original order for ties.
			let availableEntries: [(tag: Tag, canonical: Tag, maximized: Tag)] = available.map {
				let c = $0.canonical
...
internal extension Fluent.Tag.Components {
